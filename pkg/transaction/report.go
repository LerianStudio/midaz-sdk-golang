...
type GenerationReport struct {
    ...
    PhaseTimings          map[string]string   `json:"phaseTimings,omitempty"` // human-friendly durations
...
func (r *GenerationReport) SaveHTML(path string) error {
    ...
    if len(r.PhaseTimings) > 0 {
        _, _ = fmt.Fprintf(b, "<div class=\"section\"><h2>Phase Timings</h2><table><tbody>")
        for k, v := range r.PhaseTimings {
