...
- `midaz.WithRetries(maxRetries int, initialDelay, maxDelay time.Duration)` - Configures retry behavior.
- `midaz.WithCustomRetryPolicy(func(*http.Response, error) bool)` - Uses a custom retry predicate. Returning `true` marks that response/error retryable inside the SDK retry guard; unsafe methods require idempotency protection, which is auto-generated by default unless auto-idempotency is disabled or caller-provided via context/input.
- `midaz.DisableRetries()` - Disables client retries.
- `midaz.WithDebug(bool)` - Enables or disables debug logging.
- `midaz.WithTenantID(string)` - Sets a default tenant ID on entity clients.
- `midaz.WithContext(context.Context)` - Sets the client base context.
- `midaz.WithObservability(tracing, metrics, logging bool)` - Enables SDK-created observability components.
- `midaz.WithObservabilityOptions(...observability.Option)` - Configures SDK-created observability.
- `midaz.WithObservabilityProvider(observability.Provider)` - Uses an existing observability provider.
- `midaz.WithCollectorEndpoint(string)` - Configures the OTLP collector endpoint for SDK-created observability.
- `midaz.WithAccessManager(midaz.AccessManager)` - Configures Access Manager auth.
- `midaz.WithAnonymous()` - Explicitly opts out of authentication for local development and tests.
