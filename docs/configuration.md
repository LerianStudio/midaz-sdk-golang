...
_, err := client.V1.Transactions.CreateJSON(ctx, orgID, ledgerID, input)
```
...
list, _ := client.V2.Accounts.List(ctx, orgID, ledgerID, opts)

// Hard-delete instead of soft-delete (irreversible; admin only):
ctx := sdkctx.WithHardDelete(context.Background(), true)
err := client.V2.Accounts.Delete(ctx, orgID, ledgerID, accountID)
```
