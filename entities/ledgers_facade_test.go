...
// TestLedgersFacade_Count HEADs the org-scoped metrics/count endpoint and reads
// the total from X-Total-Count.
func TestLedgersFacade_Count(t *testing.T) {
	...
	n, err := newTestLedgersFacade(t, srv).Count(context.Background(), ledgersOrgID)
	if err != nil {
	...
	if want := "/v1/organizations/" + ledgersOrgID + "/ledgers/metrics/count"; gotPath != want {
...
// TestLedgersFacade_CountErrorEmptyBody proves the readCount error path maps a
// headers-only 403 (JSON content-type, empty body) to authorization, not
// internal.
func TestLedgersFacade_CountErrorEmptyBody(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/problem+json")
	...
	_, err := newTestLedgersFacade(t, srv).Count(context.Background(), ledgersOrgID)
	if err == nil {
