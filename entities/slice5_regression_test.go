...
func TestSlice5DirectConstructors_CopyTransactionBaseURLs(t *testing.T) {
...
func TestSlice5RouteServices_HTTPContracts(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NotContains(t, seen.Header.Get("X-Midaz-Auto-Idempotency"), "true")
		})
	}
}

func TestSlice5TransactionsCount_WhitelistsContractFilters(t *testing.T) {
	var seen *http.Request
...
func TestSlice5TransactionIdempotencyKey_AllowsRetry(t *testing.T) {
	var calls atomic.Int32

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "caller-key", r.Header.Get("X-Idempotency"))
		assert.Empty(t, r.Header.Get("X-Midaz-Caller-Idempotency"))
...
func TestSlice5DSLFileValidation_RejectsBeforeNetwork(t *testing.T) {
	var calls atomic.Int32
...
func TestSlice5ParseErrorResponse_RedactsTextualEnvelopeFields(t *testing.T) {
